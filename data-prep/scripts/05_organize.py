...
# Sort by country to ensure all records for a country are contiguous
radio = radio.sort_values("country")
...
with open(DATA_OUTPUT, "wb") as f_out:
    # Group by country
    for country, group in radio.groupby("country"):
        start_byte = current_offset

        # Write each station in the country group as a JSON line
        ...
        end_byte = current_offset - 1  # End of the last line for this country

        # Store offsets and the number of stations (to help the LCG)
        index_map[country] = {"start": start_byte, "end": end_byte, "count": len(group)}
...
console.print(
    f"\n[bold green]Success![/bold green] Map created for {len(index_map)} countries."
)
