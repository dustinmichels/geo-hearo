...
import { Globe, Loader2, Map as MapIcon, RefreshCw, ZoomIn, ZoomOut } from "lucide-vue-next";
...
watch(
  ...
  (newGuesses) => {
    ...
    const filter: maplibregl.FilterSpecification =
      newGuesses && newGuesses.length > 0 ? ["in", "ADMIN", ...newGuesses] : ["in", "ADMIN", ""];
...
watch(
  () => props.guessColors,
  (newColors) => {
    if (!map.value || !map.value.getLayer("countries-guessed") || !newColors) return;
...
const setTilesVisibility = (visible: boolean) => {
  ...
  if (map.value.getLayer("countries-secret")) {
    ...
    map.value.setPaintProperty("countries-secret", "fill-opacity", visible ? 0.2 : 1);
  }
  ...
  if (map.value.getLayer("countries-secret-border")) {
    ...
    map.value.setPaintProperty("countries-secret-border", "line-width", visible ? 5 : 1.5);
  }
};
