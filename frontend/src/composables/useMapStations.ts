...
export function useMapStations(
  ...
  const buildStationFeatures = (activeId?: string, zoom?: number) => {
    ...
    const features = props.stations.map((s, i) => {
      ...
      const variableHeight = s.place_size ? Math.min(s.place_size * 100, 2000000) : 600000;
      ...
      if (group.length > 1) {
        ...
        lon += (offsetKm / (111.32 * Math.cos((s.geo_lat * Math.PI) / 180))) * Math.cos(angle);
      }
  ...
  const setStationsVisibility = (visible: boolean) => {
    ...
    map.value.setLayoutProperty("stations-layer", "visibility", visible ? "visible" : "none");
  };
  ...
  const updateStationsLayer = () => {
    ...
    if (map.value.getLayer("stations-layer")) map.value.removeLayer("stations-layer");
    if (map.value.getSource("stations-source")) map.value.removeSource("stations-source");
